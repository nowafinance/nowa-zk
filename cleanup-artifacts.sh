...
run_ids=$(gh run list --repo tannetwork/nowa-zk --limit 100 --json databaseId,createdAt,conclusion --jq '.[] | select(.createdAt < (now - 604800 | todate)) | .databaseId')
...
for run_id in $run_ids; do
    ...
    gh run delete "$run_id" --repo tannetwork/nowa-zk
done
