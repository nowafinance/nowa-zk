...
setup: install-swag swagger swagger-prover build-prover
	...
	@cd contracts && forge fmt src/generated/StateVerifier.sol
...
deploy:
	@if [ ! -f contracts/src/generated/StateVerifier.sol ]; then \
		echo "⚠️  StateVerifier.sol not found. Running setup..."; \
		$(MAKE) setup; \
	fi
