...
contract BatchRegistryTest is Test {
    ...
    function setUp() public {
        ...
        batchRegistry = new BatchRegistry(address(verifier), address(stateManager), sequencer, FINALIZATION_DELAY);
    ...
    function test_Deployment() public view {
        ...
        assertEq(batchRegistry.finalizationDelay(), FINALIZATION_DELAY);
    ...
    function testRevert_DeploymentWithZeroVerifier() public {
        ...
        new BatchRegistry(address(0), address(stateManager), sequencer, FINALIZATION_DELAY);
    }
    ...
    function testRevert_DeploymentWithZeroStateManager() public {
        ...
        new BatchRegistry(address(verifier), address(0), sequencer, FINALIZATION_DELAY);
    }

    function testRevert_DeploymentWithZeroSequencer() public {
        vm.expectRevert("BatchRegistry: sequencer cannot be zero address");
        new BatchRegistry(address(verifier), address(stateManager), address(0), FINALIZATION_DELAY);
    }

    function testRevert_DeploymentWithInvalidFinalizationDelay() public {
        vm.expectRevert("BatchRegistry: invalid finalization delay");
        new BatchRegistry(address(verifier), address(stateManager), sequencer, 30 minutes);
    }
    ...
    function test_RegisterBatch() public {
        ...
        uint256[3] memory publicInputs = [uint256(INITIAL_STATE_ROOT), uint256(newStateRoot), uint256(1)];
        ...
        assertEq(uint8(batch.status), uint8(BatchRegistry.BatchStatus.Verified));
    ...
    function test_FinalizeBatch() public {
        ...
        // Fast forward time
        vm.warp(block.timestamp + FINALIZATION_DELAY + 1);

        // Finalize
        vm.expectEmit(true, true, false, false);
        emit BatchFinalized(1, newStateRoot);

        batchRegistry.finalizeBatch(1);
    ...
    function testRevert_FinalizeBatch_NotVerified() public {
        vm.expectRevert("BatchRegistry: batch not verified");
        batchRegistry.finalizeBatch(1);
    }

    function testRevert_FinalizeBatch_DelayNotMet() public {
        // Register batch
        bytes memory batchData = abi.encode("test");
        bytes32 batchHash = keccak256(batchData);
        bytes32 newStateRoot = bytes32(uint256(2));
        ...
        vm.expectRevert("BatchRegistry: finalization delay not met");
        batchRegistry.finalizeBatch(1);
    }
    ...
    function test_SequentialFinalization() public {
        ...
        uint256[2][2] memory b = [[uint256(3), uint256(4)], [uint256(5), uint256(6)]];
        uint256[2] memory c = [uint256(7), uint256(8)];
        uint256[3] memory publicInputs1 = [uint256(oldStateRoot1), uint256(newStateRoot1), uint256(1)];
        ...
        bytes32 oldStateRoot2 = stateManager.getCurrentStateRoot();
        bytes32 newStateRoot2 = bytes32(uint256(keccak256(abi.encode("newStateRoot", 2))));

        uint256[3] memory publicInputs2 = [uint256(oldStateRoot2), uint256(newStateRoot2), uint256(2)];

        vm.prank(sequencer);
        batchRegistry.registerBatch(batchHash2, newStateRoot2, batchData2, a, b, c, publicInputs2);

        // Try to finalize batch 2 before batch 1 (should revert)
        vm.warp(block.timestamp + FINALIZATION_DELAY + 1);
        vm.expectRevert("BatchRegistry: previous batch not finalized");
        batchRegistry.finalizeBatch(2);
    ...
    function testFuzz_RegisterBatch(bytes32 seed, uint256 count) public {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            uint256[2][2] memory b = [[uint256(3), uint256(4)], [uint256(5), uint256(6)]];
            uint256[2] memory c = [uint256(7), uint256(8)];
            uint256[3] memory publicInputs = [uint256(oldStateRoot), uint256(newStateRoot), uint256(i + 1)];

            vm.prank(sequencer);
            uint256 batchNumber = batchRegistry.registerBatch(batchHash, newStateRoot, batchData, a, b, c, publicInputs);
            assertEq(batchNumber, i + 1);

            // Finalize the batch so next iteration can use the new state root
            vm.warp(block.timestamp + FINALIZATION_DELAY + 1);
            batchRegistry.finalizeBatch(batchNumber);
        }
    }
    ...
    function test_CanFinalizeBatch() public {
        // Register batch
        bytes memory batchData = abi.encode("test");
        bytes32 batchHash = keccak256(batchData);
        bytes32 newStateRoot = bytes32(uint256(2));
        ...
        assertFalse(batchRegistry.canFinalizeBatch(1));

        // Fast forward
        vm.warp(block.timestamp + FINALIZATION_DELAY + 1);

        // Should be finalizable now
        assertTrue(batchRegistry.canFinalizeBatch(1));
    }

    function test_TimeUntilFinalization() public {
        // Register batch
        bytes memory batchData = abi.encode("test");
        bytes32 batchHash = keccak256(batchData);
        bytes32 newStateRoot = bytes32(uint256(2));
        ...
        uint256 timeRemaining = batchRegistry.timeUntilFinalization(1);
        assertGt(timeRemaining, 0);
        assertLe(timeRemaining, FINALIZATION_DELAY);

        // Fast forward partway
        vm.warp(block.timestamp + FINALIZATION_DELAY / 2);
        uint256 timeRemaining2 = batchRegistry.timeUntilFinalization(1);
        assertLt(timeRemaining2, timeRemaining);

        // Fast forward past delay
        vm.warp(block.timestamp + FINALIZATION_DELAY);
        assertEq(batchRegistry.timeUntilFinalization(1), 0);

        // Test with non-existent batch
        assertEq(batchRegistry.timeUntilFinalization(999), 0);
    }
    ...
    function test_UpdateFinalizationDelay() public {
        uint256 newDelay = 14 days;
        batchRegistry.updateFinalizationDelay(newDelay);
        assertEq(batchRegistry.finalizationDelay(), newDelay);
    }

    function testRevert_UpdateFinalizationDelay_NotOwner() public {
        vm.prank(user);
        vm.expectRevert("BatchRegistry: caller is not the owner");
        batchRegistry.updateFinalizationDelay(14 days);
    }

    function testRevert_UpdateFinalizationDelay_InvalidDelay() public {
        vm.expectRevert("BatchRegistry: invalid finalization delay");
        batchRegistry.updateFinalizationDelay(30 minutes); // Too short

        vm.expectRevert("BatchRegistry: invalid finalization delay");
        batchRegistry.updateFinalizationDelay(31 days); // Too long
    }
