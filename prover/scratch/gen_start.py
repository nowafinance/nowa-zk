...
import (
	...
	seqTypes "github.com/nowafinance/nowa-zk/sequencer/internal/types"
...
func fetchLatestBatch(indexerURL string) (*seqTypes.ZKBatch, error) {
...
func generateProof(batch *seqTypes.ZKBatch, ccs constraint.ConstraintSystem, pk groth16.ProvingKey) (groth16.Proof, witness.Witness, error) {
	...
	for i := 0; i < circuits.BatchSize; i++ {
		var op seqTypes.StateTransition
		...
		// Maker
		circuit.Ops[i].MakerPubKey.X, _ = new(big.Int).SetString(op.MakerPubKeyX, 10)
		circuit.Ops[i].MakerPubKey.Y, _ = new(big.Int).SetString(op.MakerPubKeyY, 10)
		...
		// Taker
		circuit.Ops[i].TakerPubKey.X, _ = new(big.Int).SetString(op.TakerPubKeyX, 10)
		circuit.Ops[i].TakerPubKey.Y, _ = new(big.Int).SetString(op.TakerPubKeyY, 10)
...
func submitProof(client *ethclient.Client, auth *bind.TransactOpts, contractAddr string, batch *seqTypes.ZKBatch, proof groth16.Proof, publicWitness witness.Witness) (*types.Transaction, *types.Receipt, error) {
	...
	const abiJSON = `[{"inputs":[{"internalType":"uint256[8]","name":"proof","type":"uint256[8]"},{"internalType":"bytes32","name":"_oldRoot","type":"bytes32"},{"internalType":"bytes32","name":"_newRoot","type":"bytes32"},{"internalType":"bytes32","name":"_withdrawalHash","type":"bytes32"},{"internalType":"bytes32","name":"_depositHash","type":"bytes32"}],"name":"submitBatch","outputs":[],"stateMutability":"nonpayable","type":"function"}]`
	...
	tx, err := contract.Transact(auth, "submitBatch", proof8, oldRoot, newRoot, wHash, dHash)
	if err != nil { return nil, nil, err }
